...
import { configFlagRefusal, validateScanOptions } from "./scan-options.js";
import {
  applySuppressions,
...
const VALID_JOURNEY_FORMATS: ReadonlySet<string> = new Set(JOURNEY_FORMATS);
/**
...
const VALID_FAIL_ON: ReadonlySet<string> = new Set(FAIL_ON_SEVERITIES);
const VALID_RULES_FORMATS: ReadonlySet<string> = new Set(RULE_LISTING_FORMATS);
const VALID_RUNTIMES: ReadonlySet<string> = new Set(RUNTIMES);
...
/**
 * Every command and every flag, attached from the one place they are declared.
 *
 * The flags used to be written here, which made removing one indistinguishable from editing one in
 * a diff — and the CLI's flags are a public surface that
 * [compatibility](../../../docs/reference/compatibility.md) says a major version has to move for.
 * They live in `cli-surface.ts` now, and `scripts/generate-cli-inventory.mjs` builds this same
 * program without actions to record what a consumer can depend on.
 */
const commands = applyCliSurface(program, {
  riskIndexModelVersions: RISK_INDEX_MODEL_VERSIONS,
  defaultRiskIndexModel: DEFAULT_RISK_INDEX_MODEL_VERSION,
});

/**
 * The command to hang an action on, or a failure at load rather than a flag missing at runtime.
 *
 * A command renamed in the surface and not here would otherwise produce a program that parses the
 * new name and does nothing when it is used.
 */
function commandNamed(name: string): Command {
  const command = commands.get(name);
  if (!command) throw new Error(`cli-surface.ts declares no "${name}" command`);
  return command;
}

commandNamed("scan").action(async (path: string, options: ScanCliOptions, command: Command) => {
    ...
    if (isStdin) {
      ...
      for await (const chunk of process.stdin) {
        ...
        if (totalBytes > MAX_STDIN_BYTES) {
          process.stderr.write(`fairux: stdin exceeds ${MAX_STDIN_BYTES} byte limit — aborting\n`);
          process.exitCode = 1;
          return;
        }
 ...
 */
commandNamed("scan-journey").action(async (file: string, options: ScanJourneyCliOptions) => {
...
commandNamed("rules").action(async (options: RulesCliOptions) => {
...
commandNamed("explain").action(async (ruleId: string, options: RulesCliOptions) => {
