...
> Calibrated against 36 pages this project wrote. Separation on them is not evidence about pages nobody here has seen.
...
| …of those, the rules stayed quiet on | 20 |
...
`adversarial-neutral-decline-plan-ja` (6) — labelled clean, and scored on findings that should not exist. The arithmetic is right and the input is wrong, which is a precision problem and not a scoring one.
