...
/**
 * Inline markdown links: `[text](target)`, and the image form. Reference-style definitions
 * (`[label]: target`) are matched separately below, because a broken one is invisible until a
 * reader clicks it.
 */
const LINK_PATTERN = /!?\[(?:[^\]]*)\]\(\s*<?([^)\s>]+)>?(?:\s+"[^"]*")?\s*\)/g;
const LINK_DEFINITION_PATTERN = /^\[[^\]]+\]:\s*<?(\S+)>?/gm;

/** A target this check has nothing to say about: it leaves the repository, or names a place in it. */
function isLocalLink(target) {
  if (/^[a-z][a-z0-9+.-]*:/i.test(target)) return false; // http:, https:, mailto:, tel:
  if (target.startsWith("//")) return false; // protocol-relative
  if (target.startsWith("#")) return false; // an anchor in this same document
  return true;
}
...
for (const file of markdownFiles()) {
  ...
  // Relative markdown links, resolved against the document that writes them. This replaced a CI job
  // that downloaded a 6MB binary from a GitHub release to do the same offline check: that job was 6
  // seconds most of the time and 38 once, which made it the slowest job in the run and the only one
  // whose duration depended on a download from outside Actions' own infrastructure.
  for (const pattern of [LINK_PATTERN, LINK_DEFINITION_PATTERN]) {
    for (const match of text.matchAll(pattern)) {
      const target = match[1];
      if (!isLocalLink(target)) continue;
      links += 1;
      // The fragment is dropped rather than checked: a heading anchor is generated by whatever
      // renders the page, and guessing that generation is how a link checker starts lying.
      const [path] = target.split("#");
      if (path === "") continue; // `[text](#anchor)` written as `[text](#a)` is handled above
      const resolved = decodeURIComponent(path);
      const target_ = resolved.startsWith("/")
        ? join(ROOT, resolved.slice(1))
        : join(ROOT, dirname(file), resolved);
      if (existsSync(target_)) continue;
      failures.push(`${file}: [..](${target}) points at nothing`);
    }
  }
}
...
console.log(
  `✓ ${checked} command and path references and ${links} relative links in documents all resolve`,
);
