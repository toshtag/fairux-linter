...
- **SARIF no longer carries a FairUX-generated `primaryLocationLineHash`.** `@fairux/report` emitted
  ...
  identity rather than the line-drift-tolerant identity GitHub code scanning matches alerts on. Because
  the field was present, `github/codeql-action/upload-sarif` did not attempt its own
  ([issue #78](https://github.com/toshtag/fairux-linter/issues/78)). The field is now absent, allowing
  the Action to attempt its source-aware fingerprint generation instead. The Action can populate the
  key only when it can resolve the primary physical source location and its line number; this change
  does not guarantee that every physical result receives a GitHub fingerprint, and this repository does
  not test an actual code-scanning upload. Logical-only DOM/Figma results have no source location to
  hash at all, and uploads that bypass the Action for the code-scanning REST API get no substitute from
  FairUX. Unchanged: `fingerprints.fairuxV1` and its algorithm, physical and logical location mapping,
  severity mapping, rule metadata, and the JSON and Markdown reporters.
