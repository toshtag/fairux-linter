 ...
 *
 * The SDK workflow used `if gh release view … >/dev/null 2>&1` for this, which conflates every one
 * of those failures with absence, and then created or edited with a fixed `--prerelease`. Both were
 * fine while every SDK release was a beta and are not once one is stable.
 *
...
import {
  ...
  SDK_RELEASE_VIEW_FIELDS,
} from "./sdk-github-release-contract.mjs";
...
const USAGE =
  "Usage: verify-existing-sdk-release.mjs --tag <tag> --prerelease true|false " +
...
if (prereleaseRaw !== "true" && prereleaseRaw !== "false") {
  ...
  // and a misspelled value read as falsy would expect a stable Release for a prerelease.
...
function readExistingRelease() {
  ...
  try {
    stdout = runSync("gh", [
      ...
      `{${SDK_RELEASE_VIEW_FIELDS.map((field) => {
